...
ffmpeg -i $FILENAME -an -vf "select=between(n\,"$FRAMES"\,"$FRAMEE"),setpts=PTS-STARTPTS" "$DIRNAME$COMPLEMENTO"/"$DIRNAME"_"$FRAMES"_"$FRAMEE".mp4 -loglevel quiet -nostats -hide_banner
echo "------------> !!!!!! Done !!!!!"